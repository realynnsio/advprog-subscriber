...
use crosstown_bus::{CrosstownBus, MessageHandler, HandleError}; use std::{thread, time}; 
...
impl MessageHandler<UserCreatedEventMessage> for UserCreatedHandler { 
     fn handle(&self, message: Box<UserCreatedEventMessage> 
        ...
        println!("In Alma Putri Nashrida’s Computer [2206814671]. Message received: {:?}", message); 
        Ok(()) 
    }
